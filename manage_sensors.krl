ruleset manage_sensors {
    ...
    fired {
      raise wrangler event "child_creation"
        attributes { "name": nameFromID(sensor_id),
                     "color": "#ffff00",
                     "sensor_id": sensor_id,
                     "rids": ["temperature_store", "wovyn_base", "sensor_profile", "subscription_manager"] }
    }
  }
  ...
  rule subscription_added {
    ...
    pre {
      ...
      new_obj = {
        "sensor_id": sensor_id, "Id": id, "Rx_role": rx_role
      }
    }
    always {
  ...
  rule auto_accept {
    ...
    pre {
      acceptable = (event:attr("Rx_role")=="Sensor"
                && event:attr("Tx_role") == "Collection").klog("Acceptable");
      new_obj = {
         "sensor_id": event:attr("name"), "Id": event:attr("Id"), "Rx_role": event:attr("Tx_role")
      }
      tx = event:attr("Tx")
    }
